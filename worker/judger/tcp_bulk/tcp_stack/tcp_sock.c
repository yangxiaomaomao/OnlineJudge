...
struct tcp_sock *alloc_tcp_sock()
{
	...
	tsk->wait_connect = alloc_wait_struct(&tsk->lock);
	tsk->wait_accept = alloc_wait_struct(&tsk->lock);
	tsk->wait_recv = alloc_wait_struct(&tsk->lock);
	tsk->wait_send = alloc_wait_struct(&tsk->lock);
...
struct tcp_sock *tcp_sock_lookup_listen(u32 saddr, u16 sport)
{
	...
	list_for_each_entry(tsk, list, hash_list) {
		if ((!tsk->sk_sip || tsk->sk_sip == saddr) &&
				tsk->sk_sport == sport)
			return tsk;
	}
...
// 2. hash the tcp sock into bind_table and established_table;
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	// lookup the forwarding entry to determine the source ip address of this
	// tcp sock
	rt_entry_t *entry = longest_prefix_match(tsk->sk_dip);
	if (!entry) {
		err = -1;
		log(ERROR, "Could not find the route entry to destination ("IP_FMT":%hu)", \
				HOST_IP_FMT_STR(tsk->sk_dip), tsk->sk_dport);
		goto out;
	}
...
int tcp_sock_listen(struct tcp_sock *tsk, int backlog)
{
	...
	if (!tsk->sk_sport) {
		log(ERROR, "could not listen to this socket due to no port specified");
		err = -1;
		goto out;
	}

	if (backlog > TCP_MAX_BACKLOG) {
		log(ERROR, "tcp_listen backlog should be no greater than %d", TCP_MAX_BACKLOG);
...
int tcp_sock_read(struct tcp_sock *tsk, char *buf, int len)
{
	...
	while (ring_buffer_empty(tsk->rcv_buf)) {
		if (sleep_on(tsk->wait_recv) < 0) {
			log(DEBUG, "sleep_on return negative value.");
			goto end;
		}
	}

	read = read_ring_buffer(tsk->rcv_buf, buf, len);

	// log(DEBUG, "tcp_sock read %d bytes of data.", read);

	int send_ack = (tsk->rcv_wnd == 0 ? 1 : 0);
	tsk->rcv_wnd += read;
	if (send_ack)
...
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len)
{
	...
	if (tsk->state == TCP_ESTABLISHED ||
		...
		wlen = tcp_send_data(tsk, buf, len);
	}
